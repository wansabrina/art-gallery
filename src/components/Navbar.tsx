...
const Navbar = () => {
  ...
  return (
    <nav className={`px-4 sm:px-4 py-2.5 fixed w-full z-20 top-0 left-0  bg-yellow-500 ${isScrolled ? 'bg-opacity-100' : 'bg-opacity-0'} shadow-xl`} data-testid="navbar">
      <div className="container flex flex-wrap items-center justify-between mx-auto">
        ...
        <div className="items-center justify-start hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
          <ul className={`flex flex-col p-1 mt-4 rounded-lg md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 bg-yellow-500 ${isScrolled ? 'bg-opacity-100' : 'bg-opacity-0'}`}>
            <li>
              <a href="https://art-gallery-oacease.vercel.app/" className="block py-2 pl-3 pr-4 text-gray-900 rounded md:hover:bg-transparent md:hover:text-gray-800 md:p-0" aria-current="page">Home</a>
            </li>
            <li>
          ...
          <a href='RegisterForm' className="flex md:order-2">
